...
| ------------------------------------- | ------------ | -------- | ---------------------------------------------------------------- |
| `01-clases.puml`                      | Clases       | Completo | 5 colecciones MongoDB, discriminator pattern, embeds, enums      |
...
| `02c-casos-de-uso-repositorio.puml`   | Casos de Uso | Completo | M5A + M5B + M8: Repositorio, dashboard analítico, notificaciones |
| `03a-secuencia-autenticacion.puml`    | Secuencia    | Completo | Login con JWT: validación, bloqueo, generación de token          |
| `03b-secuencia-procesamiento.puml`    | Secuencia    | Completo | Pipeline: Carga → OCR → NER → Almacenamiento → Notificación      |
| `03c-secuencia-verificacion.puml`     | Secuencia    | Completo | Revisión docente + verificación/rechazo del coordinador          |
| `03d-secuencia-dashboard.puml`        | Secuencia    | Completo | Dashboard: aggregations, filtros, exportación PDF                |
| `04a-actividad-pipeline.puml`         | Actividad    | Completo | Pipeline OCR+NER con swimlanes por capa                          |
| `04b-actividad-gestion-usuarios.puml` | Actividad    | Completo | Registro, login, recuperación, gestión admin                     |
| `05-estados-ciclo-vida.puml`          | Estados      | Completo | Ciclos de vida: archivo → producto → cuenta de usuario           |
