...
| 1.9     | 2026-03-20 | Carlos A. Canabal Cordero | Compendios PDF: varios borradores por archivo con segmentación controlada (heurística + LLM opcional) y preferencia de usuario “un solo documento”                                                                                                                         |
| 2.0     | 2026-04-09 | Carlos A. Canabal Cordero | Consolidación de M9 (chat grounded con historial, selección manual de modelo y fallback automático actualizado)                                                                                                                                                            |
...
El sistema aplica **OCR** (Reconocimiento Óptico de Caracteres) y **NER** (Named Entity Recognition) para extraer metadatos estructurados desde documentos académicos probatorios (PDF, imágenes y Office: `.doc/.xls/.ppt`, `.docx/.xlsx/.pptx`, ODF), eliminando la transcripción manual y garantizando consistencia, trazabilidad y disponibilidad de evidencias para procesos de acreditación institucional. Además, contempla un **chat inteligente grounded** sobre repositorio compartido, con historial persistido, selección manual de modelo.
...
| **M2 — Carga de documentos**       | Upload de PDF, imágenes (JPG/JPEG/PNG), Office moderno (`.docx/.xlsx/.pptx`, ODF) y Office binario heredado (`.doc/.xls/.ppt`) con validación MIME, límite de 20 MB, almacenamiento en MongoDB GridFS y tracking de estado; soporte de **compendios** (varios borradores por PDF) con segmentación acotada y opción usuario “un solo documento”           |
...
| **M9 — Chat Inteligente**          | Búsqueda conversacional grounded de documentos `confirmed`, con historial persistido, enlaces autenticados, tool calling y recuperación híbrida (filtros + ampliación diagnóstica + texto OCR/nativo). Cadena automática vigente: `qwen-3-235b-a22b-instruct-2507` (Cerebras) → NVIDIA → Groq → OpenRouter; Gemini queda como opción experimental/manual. |
...
| **NER**                    | `generateText` + `Output.object` (Vercel AI SDK) + Zod; cadena por capas: Gemini (`3.1-flash-lite-preview` → `3-flash-preview` → `2.5-flash-lite` → `2.5-flash`) → NVIDIA → OpenRouter → Groq (según API keys disponibles) |
| **OCR opcional**           | Mistral OCR 3 — activable vía `OCR_PROVIDER=mistral` en `.env`                                                                                                                                                             |
| **Chat IA**                | `streamText` + tool calling (Vercel AI SDK) + `Chat` (`@ai-sdk/vue`) con recuperación grounded híbrida, selector manual persistido y fallback automático `Cerebras → NVIDIA → Groq → OpenRouter`                           |
